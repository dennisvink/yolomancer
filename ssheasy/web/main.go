...
import (
	...
	"github.com/hullarb/ssheasy/web/ws"
...
func main() {
	...
	init := js.FuncOf(func(this js.Value, args []js.Value) interface{} {
		go func() {
			...
			// An SSH exit status distinguishes a remote shell exit from a lost
			// WebSocket/network connection. Only the former closes the saved tab.
			currentSession := session
			go func() {
				err := currentSession.Wait()
				_, exited := err.(*ssh.ExitError)
				if err == nil || exited {
					if callback := js.Global().Get("remoteShellClosed"); callback.Type() == js.TypeFunction {
						callback.Invoke()
					} else {
						js.Global().Call("showReconnect", "Remote shell exited")
					}
				} else {
					js.Global().Call("showReconnect", "Connection lost")
				}
				sshCon.Close()
			}()
		}()
...
func con(host string, port int, bypassProxy bool) (net.Conn, error) {
	...
	if tab := js.Global().Get("labTabID"); tab.Type() == js.TypeString && tab.String() != "" {
		url += "?tab=" + tab.String()
	}
	if bypassProxy {
